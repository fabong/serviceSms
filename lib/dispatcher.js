 ...
 * @param {function} cb
 */
exports.subscribe = function (phone, callbackUrl, description, lang, cb) {
    if(!lang) lang = '';
    debug('Subscribe for phone %s, callbackUrl "%s", description "%s" and lang "%s"', phone, callbackUrl, description, lang);

    subscriptionManager.update({phone: phone, callbackUrl: callbackUrl}, {phone: phone, callbackUrl: callbackUrl, description: description, lang: lang}, {upsert: true}, cb);
};
 ...
 * @param {function} cb
 */
exports.unsubscribe = function (phone, callbackUrl, cb) {
    debug('Unsubscribe for phone %s and callbackUrl %s', phone, callbackUrl);

    subscriptionManager.remove({phone: phone, callbackUrl: callbackUrl}, cb);
};
...
exports.sendToCallbackUrl = function (incomingSms, subscription, cb) {
    if (!subscription || !subscription.callbackUrl) return cb();
    debug('Sending received sms from %s to %s', subscription.phone, subscription.callbackUrl);

    var params = {phone: subscription.phone, msg: incomingSms.msg, description: subscription.description, time: incomingSms.time};
    var opts = {url: subscription.callbackUrl, method:'POST', json:true, body: params};
